    ...
    try:
        with db.cursor() as cursor:
            sql = """INSERT INTO users (created_at, date_of_birth, father_lastname, father_name,firstname, gender, is_active, lastname, mother_lastname, mother_name, vat )
              VALUES (now(), %s, %s, %s,%s,1,%s,%s,%s,%s,%s)
            """
...
def insert_class_student_student(values):
    db = current_app.config['DB']
    try:
        with db.cursor() as cursor:
...
def insert_class_student_student_lessons(values):
    db = current_app.config['DB']
    try:
        with db.cursor() as cursor:
            sql = """
...
def insert_epas_registration(values, dypaId):
    db = current_app.config['DB']
    #table = "student_epas_registrations" if dypaId == 1 else "student_pepas_registrations" 
    try:
        with db.cursor() as cursor:
            if dypaId == 1:
...
def insert_saek_registration(values):
    db = current_app.config['DB']
    try:
        with db.cursor() as cursor:
...
def insert_amea_registration(values):
    db = current_app.config['DB']
    try:
        with db.cursor() as cursor:
